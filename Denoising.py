...
ax2.plot(z_mp_stat, '#FD1528', lw=2, label='Denoised: MP, statistical criterion : ' + str(len(n_mp_stat)) + ' iter.')
ax2.plot(z_mp_arb, '#000000', ls='--', lw=3, label='Denoised: MP, no criterion : ' + str(len(n_mp_arb)) + ' iter.')
plt.legend(loc='lower right', fontsize=11).get_frame().set_alpha(0)
plt.axis([0, 256, -5e-12, 5e-12])
plt.tight_layout()

#plt.savefig("Prints/1_MP_arbitrary_and_criterion.pdf", bbox_inches='tight')
...
fig = plt.figure(2)
ax1 = fig.add_subplot(211, xlabel='Time (ms)', ylabel='MEG')
ax1.plot(y_exp, '#929292', label='Noisy - during experiment')
ax1.plot(z_omp_stat, '#009EFF', lw=2, label='Denoised: OMP : ' + str(len(n_omp_stat)) + ' iter.')
ax1.plot(z_mp_stat, '#FF006F', ls='--', lw=3, label='Denoised: MP : ' + str(len(n_mp_stat)) + ' iter.')
#plt.title('Signal denoising - ' + wave_name + ' wavelets, full-level decomposition.')
plt.legend(loc='lower right', fontsize=11).get_frame().set_alpha(0)
plt.axis([0, 256, -5e-12, 5e-12])
...
ortho_basis_t = gram_schmidt(wavelet_operator_t.dot(np.identity(y_exp.size)).T).T
...
ax1.plot(z_omp_stat, '#0B0B3B', ls='--', lw=3, label='Denoised: OMP, classic dic : ' + str(len(n_omp_stat)) + ' iter.')
#plt.title('Signal denoising - ' + wave_name + ' wavelets, full-level decomposition.')
plt.legend(loc='lower right', fontsize=11).get_frame().set_alpha(0)
plt.axis([0, 256, -5e-12, 5e-12])
...
for i in xrange(1, 21):
    wavelet_op_t = DictT(level=None, name='db'+str(i))
    bas_t = gram_schmidt(wavelet_op_t.dot(np.identity(y_exp.size)).T).T
    bas_t /= np.sqrt(np.sum(bas_t ** 2, axis=0))
...
z_thr = hard_thresholding(ortho_basis_t.T, ortho_basis_t, y_exp, len(n_omp_gstat))

fig = plt.figure()
ax1 = fig.add_subplot(211, xlabel='Time (ms)', ylabel='MEG')
ax1.plot(y_exp, '#5882FA', label='Noisy signal')
ax1.plot(z_thr, '#FFBF00', lw=2, label='Denoised : Hard Thresholding.')
...
ax = fig.add_subplot(211, xlabel='Time (ms)', ylabel='MEG')
ax.plot(y_exp, '#E47777', label='Noisy signal')
ax.plot(z_simu, '#30B6F5', lw=2,
        label='Re-denoised: ' + str(len(n_simu)) + ' iter.')
ax.plot(z_mp_stat, '#0E0846', ls='--', lw=3,
        label='Denoised: ' + str(len(n_mp_stat)) + ' iter.')
plt.legend(loc='lower right', fontsize=11).get_frame().set_alpha(0)
plt.axis([0, 256, -5e-12, 5e-12])

ax = fig.add_subplot(212, xlabel='Time (ms)', ylabel='MEG')
...
#plt.savefig("Prints/9_Endog_Variance.pdf", bbox_inches='tight')
